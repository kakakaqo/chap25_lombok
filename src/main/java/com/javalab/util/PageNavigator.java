...
public class PageNavigator {
	...
	public String getPageNavigator(int totalCount, int listCount, int pagePerBlock,
		...
		if(totalCount > 0) {	//총 레코드 수가 하나라도 있어야 페이징
			 ...
			 * 존재하는 전체 페이지수보다 크게 되면 기존에 구해진 종료
			 * 페이지는 의미가 없고 실제 페이지로 종료 페이지를 바꿔줌.
			 */
			...
			/*
			 * [다음 버튼 표시 유무]
			 * 현재 페이지가 전체 페이지수보다 적으면 즉, 다음페이지가 있으며
			 * 즉 현재의 페이지 블럭이 1이고 총 페이지 블럭이 2일 경우
			 * 다음 페이지 블럭으로 갈수 있도록 > 표시 (true설정)
			 */
			...
			// 전페 페이지 블럭이 1이면 이전, 다음 모두 없음.
			if(totalNumOfBlock == 1){
				isNext = false;
				isPrev = false;
			}
			
			// 요청 페이지가 1보다 크면 << 표시해주도록 설정

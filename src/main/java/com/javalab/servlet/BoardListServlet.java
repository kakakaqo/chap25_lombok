...
public class BoardListServlet extends HttpServlet {
	...
	private BoardDao boardDao = BoardDao.getInstance(); // 다른곳에서도 이렇게 호출해서 사용
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		request.setAttribute("totalCount", totalCount); // 전체게시물수
		request.setAttribute("PageNum", pageNum); // 페이지 번호
		
		PageNavigator pageNavigator = new PageNavigator();
		// jsp 화면에 보여질 페이징 관련 문자열을 Html형태로 만들어서 반환
		String pageNums = pageNavigator.getPageNavigator(totalCount,
				board.getListCount(), board.getPagePerBlock(), Integer.parseInt(pageNum));
